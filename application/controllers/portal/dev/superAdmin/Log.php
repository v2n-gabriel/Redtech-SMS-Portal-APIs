...
//        $this->session->checkSession($headers["PORTAL_AUTH"], USER_CATEGORY_SUPERADMIN, $headers["USER_AUTH"]);
    ...
    function email(){
        ...
        $filterable = array(COL_STATUS => array("Success", "Failed"), COL_USER_ID => FILTER_API_DRIVEN);
    ...
    function smsBreakdown(){
        ...
        $where_clause = "date(date_created) >= '$start_date' and date(date_created) <= '$end_date'";
    ...
    function dailyLogCount(){
        ...
        $sms_report = $sms->read(array("select" => "count(user_id) aggregate, date(date_created) day",
            "group_by" => "date(date_created)", "order_by" => "date(date_created) desc"));
